...
bannerImage: "./images/policy-as-code-governance/policy-hierarchy-lakehouse.png"
tags:
...
END;—Apply the mask to a table column
ALTER TABLE users 
ALTER COLUMN email SET MASK masks.email_masker;—Create a row filter function for regional access
...
END;—Apply the row filter to a table
ALTER TABLE orders
...
FILTER USING (region = 'US');—Apply column-level masking policy for sensitive data
