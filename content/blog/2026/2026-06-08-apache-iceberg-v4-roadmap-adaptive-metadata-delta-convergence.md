...
Iceberg and Delta Lake have converged on similar ideas, like columnar metadata, deletion vectors, and manifest-like tracking, but maintain separate metadata formats. Teams running both formats incur duplicate maintenance tooling, incompatible catalogs, and two sets of operational procedures. Databricks publicly stated at its May 2026 press cycle: "Iceberg v4 and Delta 5.0 will converge on a unified metadata structure, ending the tradeoff between interoperability and production-ready performance."
...
- **No catalog lock-in.** Dremio's REST catalog support means it can point at any v4-compatible catalog, like Polaris, Unity Catalog, Snowflake Horizon, or Nessie, and query the same adaptive metadata trees without platform-specific configuration.
